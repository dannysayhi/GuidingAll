...
public class FindItemFragment extends Fragment implements SensorEventListener {

    static FindItemFragment mFragment;
    ...
    private static boolean showItemPosition;
    private static String itemName;
    private static String itemLocation;


    public static FindItemFragment newInstance(int fragmentPosition, User user) {
        destinationDrawable = R.drawable.goods_destination;
        FindItemFragment.user = user;

        if (!isOpened || (FindItemFragment.fragmentPosition != fragmentPosition))
            return new FindItemFragment();
        else {
            FindItemFragment.fragmentPosition = fragmentPosition;
            return mFragment;
        }
    }

    public static FindItemFragment newInstance(int fragmentPosition, User user, boolean showItemPosition, String itemLocation, String itemName) {
        destinationDrawable = R.drawable.goods_destination;
        FindItemFragment.user = user;
        FindItemFragment.showItemPosition = showItemPosition;
        FindItemFragment.itemName = itemName;
        FindItemFragment.itemLocation = itemLocation;
        if (!isOpened || (FindItemFragment.fragmentPosition != fragmentPosition))
            return new FindItemFragment();
        else {
            FindItemFragment.fragmentPosition = fragmentPosition;
                ...
                mSails.loadCloudBuilding(token, buildingId, new SAILS.OnFinishCallback() {
                    ...
                    public void onSuccess(String response) {
                        getActivity().runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                if(!showItemPosition)
                                    showGoodsListDialog(getString(R.string.loadingSuccess), getString(R.string.findItemMessage));
                    ...
                    public void onFailed(String response) {
                        ...
                        getActivity().runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast t = Toast.makeText(getActivity(), getString(R.string.loadingFailed), Toast.LENGTH_SHORT);
                                t.show();
                            }
                        });
                    }
    ...
    void showGoodsListDialog(String title, String message) {
        AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
        builder.setCancelable(false)
                .setTitle(title)
                .setMessage(message)
        ...
        builder.setPositiveButton(getString(R.string.itemList), new DialogInterface.OnClickListener() {

            @Override
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
                new showItemListDialogAsync().execute();
            }
        });
    ...
    void showItemPosionDialog(String title, String message, final String itemName) {
        AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
        builder.setCancelable(false)
                .setTitle(title)
                .setMessage(message + itemName)
                .setNegativeButton(getString(R.string.no), new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        dialog.dismiss();
        ...
        builder.setPositiveButton(getString(R.string.accept), new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                mSailsMapView.getRoutingManager().setStartRegion(mSails.findRegionByLabel(user.getUserLocation()).get(0));
                mSailsMapView.getRoutingManager().setTargetRegion(mSails.findRegionByLabel(itemLocation).get(0));
                mSailsMapView.getMarkerManager().setLocationRegionMarker(mSails.findRegionByLabel(itemLocation).get(0), Marker.boundCenter(getResources().getDrawable(R.drawable.ic_find_goods)));
    ...
    void showFindItemAlertDialog(String title, String message) {
        AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
        builder.setCancelable(false)
                .setTitle(title)
                .setMessage(message)
        ...
        builder.setPositiveButton(getString(R.string.accept), new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                try {
                    ...
                    sendObject.put(JSON.KEY_STATUS, JSON.STATUS_REQUEST_ITEM_SEARCH);
                    sendObject.put(JSON.KEY_USERNAME, user.getUserName());

                    //TODO: 未來需要完成code配對物品部分, UUID, MAJOR, MINOR從手機SQLite讀取
                    sendObject.put(JSON.KEY_ITEM_UUID, Config.BT4_UUID);
                    sendObject.put(JSON.KEY_ITEM_MAJOR, Config.BT4_MAJOR);
                    sendObject.put(JSON.KEY_ITEM_MINOR, Config.BT4_MINOR);
    ...
    View.OnClickListener controlListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (v == zoomIn) {
                ...
                if(!showItemPosition) {
                    showGoodsListDialog(getString(R.string.findConnectedItem), getString(R.string.findConnectedItemMessage));
                }
                if(showItemPosition && !itemName.equals(null)) {
                    showItemPosionDialog(getString(R.string.hereWeGo), getString(R.string.immediatelyPlanThePath), itemName);
                }
            }
        }
    };
    ...
    private class showItemListDialogAsync extends AsyncTask<Void, Void, ArrayAdapter<String>> {
        ...
        protected void onPreExecute() {
            ...
            pDialog.setMessage(getString(R.string.loadingRegisteredItem));
        ...
        protected ArrayAdapter<String> doInBackground(Void... params) {
            ...
            try {
                sendObject = new JSONObject();
                sendObject.put(JSON.KEY_STATUS, JSON.STATUS_REQUEST_GET_USER_ITEM_LIST);
                user.send(sendObject.toString());
            } catch (JSONException e) {
                e.printStackTrace();
            }
            ...
            while(true) {
                if (!((receiveMsg = ReceiveMsgSession.userItemListReceiveMsg) == null)) {
                    ReceiveMsgSession.userItemListReceiveMsg = null;
                    break;
                }
            }
            ...
            try {
                ...
                if(status == JSON.STATUS_REQUEST_GET_USER_ITEM_LIST) {
                    JSONArray userArray = receiveObject.getJSONArray(JSON.KEY_ITEM);

                    for(int i = 0 ; i < userArray.length() ; i++) {
                        JSONObject detailObject = (JSONObject)userArray.get(i);
                        adapter.add(detailObject.getString(JSON.KEY_ITEM_NAME));
                    }
        ...
        protected void onPostExecute(final ArrayAdapter<String> adapter) {
            ...
            AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
            builder.setCancelable(false)
                    .setTitle(getString(R.string.registeredItem))
                    .setAdapter(adapter, new DialogInterface.OnClickListener() {

                        @Override
                        public void onClick(DialogInterface dialog, int which) {
                            dialog.dismiss();
                            showFindItemAlertDialog(getString(R.string.findItem), getString(R.string.findConnectedItemMessage) + adapter.getItem(which));
                        }
                    }).create();
            ...
            if(adapter.getCount()> 0)
                ...
                getAlertDialog(getString(R.string.nothing), "No registered item").show();
